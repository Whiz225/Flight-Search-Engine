...
const PriceGraph = ({ priceData, filters }) => {
  ...
  const CustomTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
      return (
        <div className="bg-white p-3 xs:p-4 rounded-lg xs:rounded-xl shadow-2xl border border-gray-200">
          <p className="font-semibold text-gray-800 mb-2 text-sm xs:text-base">{`${label}`}</p>
          <div className="space-y-1">
            <p className="flex items-center">
              <span className="w-2 h-2 xs:w-3 xs:h-3 bg-primary-500 rounded-full mr-2"></span>
              <span className="text-gray-600 text-xxs xs:text-sm">
                Current:{" "}
              </span>
              <span className="font-bold text-primary-600 ml-1 text-sm xs:text-base">
                ${payload[0].value.toFixed(1)}
              </span>
            </p>
            <p className="flex items-center">
              <span className="w-2 h-2 xs:w-3 xs:h-3 bg-green-500 rounded-full mr-2"></span>
              <span className="text-gray-600 text-xxs xs:text-sm">
                Average:{" "}
              </span>
              <span className="font-bold text-green-600 ml-1 text-sm xs:text-base">
                ${payload[1]?.value?.toFixed(1) || "0.0"}
              </span>
            </p>
            <p className="flex items-center">
              <span className="w-2 h-2 xs:w-3 xs:h-3 bg-red-500 rounded-full mr-2"></span>
              <span className="text-gray-600 text-xxs xs:text-sm">
                Minimum:{" "}
              </span>
              <span className="font-bold text-red-600 ml-1 text-sm xs:text-base">
  ...
  return (
    <div className="card overflow-x-auto">
      {/* Header */}
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-4 xs:mb-6">
        <div className="flex items-center mb-3 xs:mb-4 md:mb-0">
          <div className="p-2 xs:p-3 bg-primary-100 rounded-lg xs:rounded-xl mr-3 xs:mr-4">
            <FaChartLine className="text-xl xs:text-2xl text-primary-600" />
          </div>
          <div>
        ...
        {/* Stats */}
        <div className="flex flex-wrap gap-2 xs:gap-3 md:gap-6">
          <div className="text-center">
            <div className="text-xxs xs:text-sm text-gray-600 mb-1">
              Current Filter
            </div>
            <div className="text-lg xs:text-xl md:text-2xl font-bold text-primary-600">
              ${filters.maxPrice}
            </div>
          </div>
          <div className="text-center">
            ...
            <div className="text-lg xs:text-xl md:text-2xl font-bold text-green-600">
          ...
          <div className="text-center">
            ...
            <div className="text-lg xs:text-xl md:text-2xl font-bold text-red-600">
              ${minPrice.toFixed(1)}
            </div>
          </div>
        </div>
      ...
      <div className="h-48 xs:h-56 sm:h-64 md:h-72 lg:h-80 mb-4 xs:mb-6">
        <ResponsiveContainer width="100%" height="100%" minWidth={300}>
          ...
          >
            ...
            <Area
              ...
              strokeWidth={3}
              name="Current"
              dot={{ stroke: "#6366f1", strokeWidth: 3, r: 4 }}
            />
          </AreaChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="flex flex-wrap justify-center gap-2 xs:gap-3 md:gap-4 mb-3 xs:mb-4">
        <div className="flex items-center">
          <div className="w-2 h-2 xs:w-3 xs:h-3 bg-primary-500 rounded mr-1 xs:mr-2"></div>
          <span className="text-xxs xs:text-xs font-medium text-gray-700">
            Current
          </span>
        </div>
        <div className="flex items-center">
          <div className="w-2 h-2 xs:w-3 xs:h-3 bg-green-500 rounded mr-1 xs:mr-2"></div>
          <span className="text-xxs xs:text-xs font-medium text-gray-700">
            Average
          </span>
        </div>
        <div className="flex items-center">
          <div className="w-2 h-2 xs:w-3 xs:h-3 bg-red-500 rounded mr-1 xs:mr-2"></div>
          <span className="text-xxs xs:text-xs font-medium text-gray-700">
            Minimum
          </span>
        </div>
      ...
      <div className="mt-3 xs:mt-4 pt-3 xs:pt-4 border-t border-gray-200">
        <div className="flex items-start">
          <FaInfoCircle className="text-primary-500 mt-0.5 mr-2 xs:mr-3 flex-shrink-0 text-sm xs:text-base" />
          <div>
            <h4 className="font-semibold text-gray-800 mb-1 xs:mb-2 text-sm xs:text-base">
              Price Insights
            </h4>
            <p className="text-gray-600 text-xxs xs:text-sm">
              Prices drop on Wed/Thu. Book 3-4 weeks ahead to save up to 20%.
            </p>
          </div>
        </div>
      </div>
